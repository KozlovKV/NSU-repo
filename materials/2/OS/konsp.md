...
- Перенос части данных о процессе (в частности, данных об обработчиках) в структуру `proc` из области `u` - благодаря этому ядро без пробуждения процесса сможет узнать, что тот игнорирует сигнал
...
struct sigaction {
  ...
  sigset_t  sa_mask; // Маска сигналов, которые будут заблокированы в дополнение к сигналу-триггеру
  int       sa_flags;
  /*
    Флаги, определяющиеся особенности поведения обработчика сигналов
