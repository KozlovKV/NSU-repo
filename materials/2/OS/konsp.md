...
В SVR4 также реализуются через подсистему STREAMS. Оба дескриптора ДВУНАПРАВЛЕННЫЕ, при этом фактически создаётся 2 канала с перекрёстной связью - запись по первому дескриптору направит данные для чтения по второму и наоборот.
...
*Фан-факт 4:* самый быстрый стрелок на диком западе - это `SIGKILL`, или, как его ещё называют - Номер `9`. Его нельзя перехватить, от нельзя убежать и защититься, он убьёт вас и вашу семью
...
### Обработка сигналов
**СИСТЕМА НЕ ГАРАНТИРУЕТ, ЧТО СИГНАЛЫ БУДУТ ДОСТАВЛЕНЫ ТУТ ЖЕ**, т.к. не работающий в данный момент процесс не может обработать сигнал. При этом, если процесс находится в состоянии блокировки системным вызовом (например, чтения), то при получении сигнала системный вызов прекратится с ошибкой `EINTER`

У каждого процесса есть масаа `pending` - тех сигналов, которые были отправлены процессу, но ещё не обработаны.

- `SIG_DFL` - реакция по умолчанию (смэрть)
...
handler_t signal (int sig, handler_t handler); // Назначает новый обработчик handler для сигнала sig и возвращает прошлый обработчик этого сигнала
handler_t sigset (int sig, handler_t handler); // Позволит перевести сигнал во время обработки в состояние SIG_HOLD, то есть повторные такие же сигналы будут удерживаться
```
...
Во время обработки сигнала мы почти что неизбежно попадём в критическую секцию, если в обработчике мы вызовем работающие с разделяемыми данными функции, мы рискуем получить дэдлок.

**Async signal safe** - функции, безопасные для вызова в сигналах. В первую очередь это чистые функции.

Кроме того, в обработчиках сигналов

*Дописать подробнее про `signal` и `sigset`*

### Генерация сигналов
От процесса:
- `kill(2)` - `kill(pid_t pid, int sig)` - можно посылать только процессам, реальный ID которых равен ID вызвавшего системный вызов процесса (*добавить про выбор id для групп*)
  - **Когда `kill` был вызван и даже когда он вернулся, мы не можем быть уверены, что процесс-адресат был успешно убит**. Обработка сигнала происходит в один из трёх моментов:
    - Процесс перешёл из runnable в running
    - Процесс был
- `sigsend(2)` - более сложный системный вызов, позволяющий специфицировать ID процессов-получателей и сигналы
...
### `setjmp(3)`
*nonlocal goto* - означает, что 

`setjmp()` - сохраняет состояние стэка

`longjmp()` - позволяет вернуться из одной функции в другую при условии, что мы сохранили состояние стэка, то есть мы можем прыгнуть Vtк месту вызова `setjmp`. `setjmp` вернёт ноль, если встречает в выполняемой последовательности кодов и иное значение, если вызов `longjmp` привёл к переходу к месту вызова

Механизмы, похожие на эти функции, используются при реализации примитивов синхронизации

### `sleep(3C)`
`unsigned sleep(unsigned seconds)` - безопасно для многопоточности и не влияет на обработку сигналов
...
Применение `sigset_t`:
- `int sigpending(sigset_t * set)` - позволяет узнать, какие сигналы в данный момент ждут обработки
- *Дописать про `sigprocmask`*
