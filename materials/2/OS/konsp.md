...
- [*остановился на стр. 753*](#остановился-на-стр-753)
...
Драйвера позволяют - это простой интерфейс, связывающий периферийные устройства с ядром системы. Благодаря этому разработчики могут не задумываться о технических деталях каждого конкретного устройства.
...
Как терминалы связаны с процессами?
  ...
  Если во время исполнения процесс основной группы нажать Ctr+Z, группа получит сигнал SIGTSTP, будет переведена в фоновый режим и остановлена
    Командной fb можно снова сделать группу основной. Командой bg такая замороженная группа будет продолжена в фоне.
...
**Зомби процесс** - процесс, который уже завершился, но ещё не отправил никому свой код возврата. Такой процесс будет занимать ячейку в таблице pid'ов, поэтому при накоплении зомби мы потеряем возможность создавать новые процесс. 
...
#### `waitid`
`pid_t waitid(idtype_t idtype, id_t id, siginfo_t *infop, int options)` - позволяет выбрать сразу несколько вариантов ожидания одного или нескольких процессов в зависимости от значения `idtype`:
- *добавить из мана*
...
Позволяет зарегистрировать функции, которую будут вызваны после `exit()` и до завершения процесса
...
### `ulimit(2)`
`long ulimit(int cmd, [newLimit])` - архаичная версия для изменения лимитов (сейчас используются `getrlimit` и `setrlimit`)через `open(2)` или `creat(2)` права доступа будут определены как `access = mode & ^cmask`
...
struct stat {
  ...
  off_t st_size;
  timestruc_t st_atim, st_mtim, st_ctim; // Время доступа, модификации и создания. Создание и модификация хранятся с точностью до миллисекунд, доступа - до пяти часов
};
...
- `struct passwd* getpwent()` - возвращает последовательнно все записи
...
**Эти функции не гарантируют возврат нвого указателя. Более того, зачастую они перезаписывают данные по тому же указателю**
...
### `chown(2)`
*дописать*
Могут менять владельца и группу файла (**для нас на Солярисе доступна только смена группы**)

### `utime(2)`
*дописать*
Меняет время модификации файла. Владелец может менять время на произвольное, другой пользователь с правами доступа на запись может передать нулевой нулевой указатель и установить тем самым время изменения как текущее
