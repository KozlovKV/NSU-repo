...
Для нечётких множеств также определены операции дополнения (1 минус ФП), объединения (максимум ФП) и пересечения (минимум ФП) и умножение (умножение значений ФП). Операции возможны только над НМ с одинаковым универсальным множеством.

![](./materials/lectures/23-10-27%20-%20НМ%20операции.png)

*Ещё было про нечёткие отношения и декартовы произведения, но не уверен, что это особо важно*
...
Другим слабым местом является взвешенность отдельных сведений и их использование в нечётких правилах. Если по двум разным правилам мы получили разное значение одного одной и той же буквы из УМ, как быть? Решение - **использовать коэффициенты уверенности** `КУ(h : e) = МД(h : e) - МНД(h : e)`:
...
# Ещё раз о КР
Будет 3 задачи:
1. Метод резолюции (деревья вывода?) (2,5 - 3 балла)
   1. Смотри лекцию 5. Может быть как чисто в резольвентах, так и в предикатах (с кванторами, но это уже маловероятно. Энивэй, примеры в пятой лекции + правила вывода можно посмотреть в дискретке за первый курс, но лучше на них особо не опираться, потому что, будто бы, тут всё проще)
2. 2 задачи на нечёткие модели
   1. Арифметика над нечёткими множествами
   2. Одна ищ двух задач:
      1. Дана ЛП. Нужно придумать для неё значения и ФП, нарисовать график для значений ФП
      2. Нечёткое отношение (*вот это чуть пугает, надо будет перечить из лекции*)