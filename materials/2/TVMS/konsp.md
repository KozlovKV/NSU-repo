...
$$
\frac{S_n - na_1}{\sqrt{n\sigma_1}} \rarr N_{0,1}
$$
...
**Здесь мы будем пользоваться тем фактом, что стандартное нормальное распределение будет считаться через интеграл Лапласа. То есть мы легко можем посчитать вероятность попадания множества событий в какие-то рамки, перейдя от записи $P(a < S_n < b)$ к записи $P(\frac{a - na_1}{\sqrt{n\sigma_1}} < \frac{S_n - na_1}{\sqrt{n\sigma_1}} < \frac{b - na_1}{\sqrt{n\sigma_1}})$, для которой уже и применим интеграл Лапласа. Поэтому все задачи на ЦПТ сводятся к поиску в ней ответа через нахождение вероятностей вида $P(a < S_n < b)$**
