...
# 23.10.23 - лекция
*Тут, наверное, либо 7.1, либо 7.2 - чтобы понять, придётся в пн пойти на пару...*
...
Из определения матожидания следует, что для непрерывной функции $y = g(x)$: $EY = \int_a^b f(x)g(x) dx$, где $f(x)$ - плотность функции, а $g(x)$ - определённая нами функция. Отсюда следует, что для $g(x) = x$ мы получаем $EX = \int_a^b f(x)xdx$
