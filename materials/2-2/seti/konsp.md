...
- `iptables-apply` - безопасная установка правил. В случае ситуации, когда установка правила приведёт к разрыву соединения, откатит этот изменение
  ...
  - В качестве источника данных может выступать файл либо команда после флага `-c`
    - Так, если команду для блокировки входящих сигналов выше использовать с этой утилитой: 
      ...
      - Утилита спросит, можете ли вы сейчас подключиться. В случае проблем с добавленными правилами, мы просто не сможем отправить символ `y`, поэтому через несколько секунд будет принят ответ по умолчанию `N`, который сбросит внесённые изменения
...
- Если указать `<hostAddress>`, то доступ будет открыт только при обращении извне от этого адреса (*по крайней мере, так везде пишут, на практике разницы на своих машинах не заметил*)
