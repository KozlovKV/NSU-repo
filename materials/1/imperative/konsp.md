...
## Чем полезен DFS
### Топологическая сортировка
Проводится при помощи DFS с учётом времени выхода. Далее сортируем вершины по убыванию по времени выхода и получаем топологически отсортированный массив вершин. Также можно вместо присвоения моменн=та выход сразу складывать вершины в топологоичски отсортированный массив в момент выхода из этих вершин.
...
*На лекции рассматривается метод поиска через учёт глубины в дереве и уровень поднятия `up` (значение минимальной глубины, на на которую можно перейти из данной по пути, включающем одно back-ребро), что по сути дублирует параметр вершины `low`, рассмотренный на дискретке (разница лишь в том, что там мы брали момент входа, а тут - глубину).*
