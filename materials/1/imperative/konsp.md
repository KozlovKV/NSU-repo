...
- [Бинарная куча](#бинарная-куча)
- [Быстрое возведение в степень](#быстрый-способ-возведения-в-степень)
- [Алгоритм Евклида](#алгоритм-евклида)
...
Чтобы найти такое число `x`, чтобы при заданном числе `a` и модуле `m` получалось, что `a*x % m = 1`
...
Чтобы найти взаимно простое число для `a` надо найти $a^{\phi(m)-1}$
...
Позволяет решать Диафантовы уравнения:
- `a` и `b` взаимно простые
- найти такие `x` и `y`, что `ax + by = 1`
  - Если `a` и `b` - не взаимно простые, то `ax + by = NOD(a, b)`