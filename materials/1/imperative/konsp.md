...
```
Все функции ввода-вывода работают на `vararg`'ах. При этом, чаще всего рекомендуется делать `vararg`-обёртку с точками, а инициализированный в ней `va_list` передавать внутрь функции, уже работающей с ним.
...
## Псевдошаблоны
Повторение материала [отсюда](#почти-что-шаблоны-типов). Здесь добавлю только то, что после миним практического задания в очередной раз убедился, что это невероятно неудобный метод. Если уж хочется полиморфизма, то лучше добавиться его через `void*`, как в [этом списке](#двусвязный-универсальный-список)