...
- [23.04.28 - лекция](#230428---лекция)
  - [Метод Форда-Фолкерсона](#метод-форда-фолкерсона)
  - [Поиск паросочетаний](#поиск-паросочетаний)
  - [Поиск максимального количества рёбрено-непересекающихся путей](#поиск-максимального-количества-рёбрено-непересекающихся-путей)
  - [Поиск максимального количества вершинно-непересекающихся путей](#поиск-максимального-количества-вершинно-непересекающихся-путей)
  - [Поиск минимального разреза](#поиск-минимального-разреза)
- [23.05.08 - лекция](#230508---лекция)
  - [ООП](#ооп)
  - [Обращение к памяти](#обращение-к-памяти)
- [23.05.11 - семинар](#230511---семинар)
  - [Декартовы деревья](#декартовы-деревья)
    - [Функции декартова дерева](#функции-декартова-дерева)
- [23.05.19 - лекция](#230519---лекция)
...
*На лекции рассматривается метод поиска через учёт глубины в дереве и уровень поднятия `up` (значение минимальной глубины, на которую можно перейти из данной вершины по пути, включающем одно back-ребро), что по сути дублирует параметр вершины `low`, рассмотренный на дискретке (разница лишь в том, что там мы брали момент входа, а тут - глубину).*
...
[Реализация этой версии с большим количеством статистики](#поиск-точек-сочленения)
...
# 23.04.14 - лекция
## Переборы
*Задачу с расстановкой `n` ферзей на поле `n*n` описывать не буду - рассматривали на дискретке*

## Динамическое программирование
*ХЗ, что тут писать. Ну... Полезно при решении очень многих задач, когда мы можем разделить одну большую задачу на небольшие подзадачи*

Жадный алгоритм: даны объекты с ценой и весом. Алгоритм позволяет найти максимальную цену предметов, сумма которых не превосходит максимально допустимого веса.

Часто комбинируются с мемоизацией

# 23.04.20 - семинар
## Алгоритмы с возвратом
Также называются задачами искуственного интеллекта. Основаны на переборе разных решений и проверке их корректности. Подобные задачи являются NP-полными.

Пример задачи: ход коня. Необходимо на шахматном поле `n^2` пройти конём все клетки; Выбор партнёров (*лол*)

Общий механизм всех таких задачи описывается так:
```python
# Заходя в этот шаг рекурсии будем пытаться определить ответ для i-й позиции
def tryFunc(i): 
    if (i == answerLen)
        return; # Перебор дошёл до самого конца (и не факт, что ответ был найден)
    for value in allPossibleValue:
        # Проверям, может ли значение value встать на i-ю позицию (конкретная реализация зависит от решаемой задачи)
        if (!possibleValue(i, value)):
            continue
        
        # Устанавливаем значение value для позиции i. Тут же проверяем, найден ли был полный ответ
        saveValue(i, value)
        try(i+1)
        if (solutionFound)
            return
...
Самый простой вариант получить хорошее бинарное дерево - перемешивать входные данные, чтобы они не поступали отсортированно. Но работать это будет только для задач, где нам сразу известны все входные данные.

Поворот - перемещение позиции корня на позицию одного из сыновей

### Red-Black балансировка
1. Все вершины со значениями окрашиваются в чёрный или красный
2. Красные с красными не должны быть смежными
3. При пути до любого листа количество пройденных чёрных вершин должно быть одинаковым
4. Вставка вершины:
   1. Вставляем вершину, подсоединяем 2 чёрных листа-пустышки
   2. Окрашиваем новую вершину в красный
   3. Если конфликта с родителем по цветам нет, оставляем всё как есть
   4. Если родитель красный:
      1. *Должны решать 100500 разных случаев разрешения конфликта... AVL выглядит куда приятнее*

**[Лучше вот про AVL почитать](https://habr.com/ru/articles/150732/)**

## Block-tree
Block-tree - дерево, в котором в каждой вершине хранится массив из `n` записей. От такой вершины будет исходить `n+1` рёбер. Очень удобно для хранения в постоянной памяти

# 23.04.27 - семинар
## Динамическое программирование
Суть в разбиении главной задаче на мини-задачи, считающие похожие значения. Используется совместно с мемоизацией. За счёт мемоизации мы можем превратить рекурсивный алгоритм в итеративный.

### Комбинации битов
Простейший пример: вычисление количества комбинаций из 0 и 1 длины `n`, где рядом не идут 2 единицы:
```
seq = [] # Массив, где i-я позиция - кол-во комбинаций
seq[0] = -1 # неопределено
seq[1] = 2 # 0, 1
seq[2] = 3 # 00, 01, 10

# Тогда получаем рекуррентную формулу
seq[k] = seq[k - 1] + seq[k - 2]
```

### Минимальная сумма квадратов

### Рюкзак (не для перебора)

### Упрощённый рюкзак
Надо ответить на вопрос: можно ли набрать из `n` предметов ровмно массу `w`

### Классическая задача на рюкзаке (жадный алгоритм)
**Динамику можно использовать для целых чисел**

Таблица, в которой на ячейка `t[i][j]` хранит максимальную стоимость для i первых предметов при ограничении в j кг. Есть 2 случая:
- Не берём `i`-й элемент: `t[i][j] = t[i-1][j]`
- Берём `i`-й элемент: `t[i][j] = t[i-1][j-w_i] + c_i`

Соответсвенно, для `j < w_i` просто записываем первый случай

При `j >= w_i` берём максимум из двух вариантов

### Задача делимости
Расставить знаки +/- перед заданными числами так, чтобы сумма делилась на заданное k. Таблица `[n][k]`, где на позиции `[i][j]` стоит единица, если сумма i первых чисел имеет остаток j

# 23.04.28 - лекция
Рёбра остаточного графа:
- Если в исходном графе рёбра ориентированные, то обратные им рёбра будут иметь максимальную пропусную способность 0
- Если рёбра неориентированные, то максимальная пропускная способность для прямого и обратного ребра будет одинаковой
- Для прямого и обратного ребра всегда должен соблюдаться инвариант `f[forward] = -f[backward]`, причём в обе стороны
  - **Очень важно, чтобы из какого-либо ребра мы могли быстро получить обратное ему** (*Как это лучше сделать - дело ваше. Лично мне охота использовать списки смежности с указателями на рёбра, хранящими указатели на обратные им, но можно сделать массив длины `2n`, где на позициях `2k` и `2k+1` будут храниться обратные друг-другу рёбра*)

## Метод Форда-Фолкерсона
1. Ставим нулевой поток для всех рёбер
2. Пока в остаточной сети есть путь `s-t`
   1. Берём этот путь `P`
   2. Ищём `b = bottleneck(P)` - минимальное значение пропускной способности
   3. Увеличиваем рёбра пути на `b`, а обратные им уменьшаем на `b`

## Поиск паросочетаний
Поиск максимальных паросочетаний возможен, если мы подсоединим ко всем вершинам одной доли двудольного графа исток, а ко всем вершинам другой - сток.

Задаём всем рёбрам пропускную способность `= 1`

Дальше находим максимальный поток и берём для паросочетаний насыщенные рёбра

## Поиск максимального количества рёбрено-непересекающихся путей
Назначаем всем рёбрам пропуснкую способность 1 и запускаем поиск максимального потока. Значение входного потока для стока будет равняться количеству рёберно-непересекающихся путей.

## Поиск максимального количества вершинно-непересекающихся путей
Для этой задачи необходимо сделать так, чтобы вершины как бы имели свою пропускную способность (как будто это рёбра).

Для этого разделяем каждую вершину на две, связанные ребром ориентированным - входную и выходную. Соответственное, во входную направляем все рёбра, входившие в исходную вершину, а из выходной выводим все рёбра, исходившие из исходной.

Благодаря этому мы сводим задачу к предыдущей.

## Поиск минимального разреза
Запускаем поиск максимального потока, а после него делаем обход в глубину или ширину. Все достигнутые вершины будут относиться к первому множеству, остальные - ко второму. Соответственно, разрез будет проходить через рёбра, исходящие из одного множества в другое.

# 23.05.08 - лекция
## ООП
*В C для ООП надо изобретать велосипеды... Кааайф*

*Пожалуй, базовые понятия я писать не буду. Об этом полно книг/статей + я это уже итак неплохо знаю + у нас будет об этом особый курс*

*Фан-факт:* даже в C++ к приватным полям можно обратиться извне, если воспользоваться сдвигами по памяти. По-настоящему жоска инкапсуляция из популярных языков есть только в Java

Реализация классов через структуры и методы-функции не позволяет нам внедрить инкапсуляцию, а потом используется достаточно редко и только для небольших классов. 

Для серьёзных классов в хэдэре делается лишь форвард-объявление структуры либо создание указателя `void*`, который по имени позже будет соответствовать нашей структуре. Далее идёт декларация абстрактных методов. Описание структуры и реализация методов располагаются в c-файле. Очевидно, что кроме инкапсуляции (компилятор банально не позволит обратиться к полям структуры, если она не описана в хэдэре), мы получаем ещё и полиморфизм

## Обращение к памяти
*Ну... На лекции были замеры на примерах псевдо-списка массива, а тут я, пожалуй, только оставлю результаты*

Последовательный доступ к памяти работает тем быстрее в сравнении со случайным, чем больше объём обрабатываемых данных. 

# 23.05.11 - семинар
## Декартовы деревья
Декартовы деревья прендставляют собой комбинацию кучи и дерева поиска. В каждой вершине хранится пара `(x, y)`. Дерево поиска реализуется по `x` (`x_left < x < x_right`), а куча - по `y` (`y >= y_left && y >= y_right`)

### Функции декартова дерева
- `split(T, k)` - разделяет дерево `T` по ключу `x = k` на `T_left` и `T_right`. Вершина с `x = k` попадает в левое дерево. Далее `split(T_right, k)` будет вызываться рекурсивно до тех пор, пока будет возвращаться какое-то левое дерево. Если левое дерево возвращается, мы присоединяем его как правого сына к левому дереву.
- `merge(T_1, T_2)` - возвращает слитое из `T_1` и `T_2` дерево `T`, причём все ключи в `T_1` должны быть меньше ключей в `T_2`. Корневая вершина из обоих деревьев выбирается в зависимости от её приоритета `y`. Для поддеревьев того дерева, из которого мы вытянули корень, вновь вызывается `merge`

**На основании хорошей реализации двух этих функций решаются все остальные возможные задачи**

# 23.05.19 - лекция
## Выделение памяти
Оптимизировать выделение памяти можно, если мы представляем, под какие задачи нужна память. Например, следующий алгоритм:

**Линейный `malloc`**
1. Выделяем много-много памяти (буфер)
2. Для всех объектов у нас будут участки памяти фиксированной длины
3. Храним указатель на свободную часть буфера
4. При вызове собственного malloc'а выделяем память из буфера и двигаем указатель на место после выделенной области

Возникает вопрос, как очищать память или выделять больше памяти, когда весь буфер уже был заполнен.
 
Можно выделять несколько буферов и связывать их двусвязным списком. Можно учищать буфер с конца, так как зачустую выделенная раньше память очищается позже и наоборот.

Можно хранить односвязный список указателей на свободные блоки. Это позволит нам получать свободные блоки линейно и очищать блоки произвольно. Очищенный блок добавляется в начало списка.

**Фрагментация** - ситуация, когда в памяти выделены небольшие куски, лежащие в разных местах. В результате мы не сможем выделить большой блок памяти, хотя по объёму памяти хватает.

Проблемы с памятью *мы знаем толк в извращениях*:
1. Обращение к `NULL`
2. Обращение в ячейку вне выделенного блока (*"out of bounds", но не для спидранеров*)
3. Чтение выделенной, но не заданной памаяти - не очень опасна. Просто у нас будут случайные значения. Если использовать `calloc`, то выделенная память будет обнуляться
4. Утечка памяти (memory leak) - **НАДО ВСЕГДА ЗА СОБОЙ УБИРАТЬСЯ**
   - Утечка памяти - частный случай утечки ресурсов. При неаккуратной организации программы можно получить утечку фаловый дескрипторов, сокетов, ресурсов периферийных устройств и т.п.
5. double free - *дорогой дневник, мне не подобрать слов, чтобы описать ту боль и унижение, которые я испытывал, создавая универсальный спсок с нормальной очисткой памяти*
6. Обращение к очищенному указателю

Как избавиться от проблем 4-6? Главный метод - **владение памятью**. Заключается метод в том, что при наличии множетсва указателей на выделенный блок памяти мы определяем, какие из них владельцы, а какие - вспомогательные. При исчезновении вспомогтаельных не происходит ничего, а при уничтожении всех владельцев должна будет осободиться и память. Из такой организацыы следует, что каждый указатель-владелец должен знать обо всех других владельцах. (*соответственно, единоличное владение реализуется проще*)
