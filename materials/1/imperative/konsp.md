...
Если над провести операции над всеми остатками заданных чисел $P$ и $Q$, полученных с использованием модулей $\vec{M}$, то итоговое число можно восстановить следующим образом:
- $P =  p_1a_1 + p_2a_2 + ... p_ka_k$
- $a_i \% m_i = 1$
- $a_i \% M_j = 0$
- $\rArr a_i = \prod_{j \not = i}(M_j) b_i$
