...
- `traverse :: (a -> f b) -> t a -> f (t b)` - принимает функтор и список, а возвращает обработанный список, обёрнутый в функтор (*пока что похоже на то, что `forM` и `mapM` работают совершенно идентично, что странно...*)
