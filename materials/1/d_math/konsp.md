...
*Ещё несколько определений связных вершин*

Слабо связным графом называется граф - ориентированный граф, который при замене направленных дуг на неориентированные рёбра, станет связным.
...
*Тут ещё много чего было. Проверить потом в лекции*
...
**Нижняя точка** - наименьший номер вершины, в которую можно попасть по бэк-ребру (при этом переход по бэк-ребру - последний шаг в поиске). (`min(cur.id, {u.low | u in children of cur}, {v.low | v in back-vertices of cur})`). Находится модификацией DFS и проверкой по полю вершины `.d`
