...
**Т. О корректности алгоритма Дейкстры** Алгоритм Дейкстры, запущенный на графе без отрицательных рёбер, даст нам кратчайшие пути из исходной вершины и построит дерево кратчайших путей. *Доказательство:* для него нам надо доказать что все добавленные в `S` вершины имеют минимальную длину пути, для чего нужно доказать, что добавляем на текущей итерации вершина `u` имеет минимальную длину пути. Для начальной вершины `s` и конечной `u`, если они в одной компоненте связности, будет существовать кратчайший путь `P`. Предположим, что на пути P содержится ребро `x-y`. `x` уже включена в `S`, а `y` - нет. (возможно `s = x` и/или `y = u`). Если `x` была добавлена в `S`, `y.d` было отрелаксировано и теперь равно `delta(s, y)` (так как это часть кратчайшего пути `P`), если в этот момент мы извлечём из очереди `u`, то получим `y.d = u.d`, что противоречит условию, значит мы сначала должны выбрать `y`, после чего `u.d` будет отрелаксировано до `delta(s, u)` и уже потом выбрана как минимальная
