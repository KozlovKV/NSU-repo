...
**Нижняя точка** - наименьший номер вершины, в которую можно попасть по бэк-ребру (при этом переход по бэк-ребру - последний шаг в поиске). (`min(cur.id, {u.low | u in children of cur}, {v.low | v in back-vertices of cur})`). Находится модификацией DFS и сравнением данных при выходе из вершины
