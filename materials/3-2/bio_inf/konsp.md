...
Формат `.fasta`:
```fasta
>sequence_1 # любые символы без пробелов кроме >
[A|T|G|C|N] # Цепочка возможно нулевой длины, N - неизвестный нуклеотид. В случае пустой цепочки нужна всё равно пустая строка
>sequence_2
...
```
Выше была нуклеотидная фаста, может быть также белковая - в ней 20 букв - по букве на аминокислоту

Брать фасты будем [отсюда](https://www.ncbi.nlm.nih.gov/)
...
Благодаря этим мутациям можно перейти от любой цепочки к любой

Сопоставление цепочек друг другу - это **выравнивание**

Для выравнивания считаем **alignment score**:
- Совпадают буквы - match - `+1`
- Разные буквы - mismatch - `-1`
- В одной из цепочек прочерк - gap - `-2`

Мы хотим найти выравнивание с наибольшим скором

Простой вариант - алгоритм динамического программирования от Нидлмана и Вуша, который очень прост, однако:
- Неэффективен - память `O(n^2)`, время `O(n^3)`
- Избыточность - находит все наиоблее выгодные выравнивания, хотя нам чаще всего достаточно одного
- Глобальность - выравнивает всю цепочку (в противовес этого мы можем выравнивать лишь часть последовательности)

# 25.04.07
При множественном выравнивании мы теперь хотим выровнять N цепочек одновременно, а не 2. mismatch и gap будут в тех случаях, когда есть на этом месте match или gap между любыми двумя цепочками

Для записи выравнивания множества цепочек используется формат CLW

# Второй семестр
# 25.10.24
## 10 нерешённых задач биоинформатики
1. Определение эволюционных взаимоотношений между белками
2. Определение локальных гомологий в белках - в целом
3. Предсказание функций белка
4. Предсказание структуры белка (игруха fold.it)
5. Определение трансмембранных белков
6. Поиск функциональных сайтов (*жаль, пояснение пропустил*)
7. Предсказание белок-белковых взаимодействий
8. _
9. Дизайн белков
10. Инжиниринг белков - то есть уже не просто его проектировать и создать, а заставить его полценно работать (а также в идеале, чтобы его строили сами клетки)