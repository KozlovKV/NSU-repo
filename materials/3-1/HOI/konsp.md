...
# 25.02.20 - Лекция
## Реляционная модель
Возникла в 70-м году и в 75-м жоска захайпила

- **Отношение** - неупорядоченное множество кортежей со схемой
- **Кортеж** - неупорядоченный набор пар `(key, value)`
- **Домен** - набор значений (тип данных)
- **Схема отношения** - кортеж пар `(name, domain)`

# 25.02.27
## OLAP vs OLTP
OLTP:
- Работ с малым количеством записей (в ходе одного запроса)
- Охота обработать достаточной быстро ("мгновенно" либо за время около секунды)
- Взаимодействует с конечным пользователем сервиса
- Работает как правило в основном с актуальными данным

OLAP: работает с большим объёмом данных, интересны не только актуальные данные, но и итсория

MOLAP - многомерный анализ, который заключается в предпосчитывании определённых метрик и статистик

CQRS (Command query reqyest separation) - на уровне БД сводится к разделению БД на аналитическую и транзакционную части

# 25.03.13
## REST
- Есть некоторые ресурсы
- Для ресурсов определены Unififed resource locator'ы (URL), по одному и тому же URL мы гарантированно получаем доступ к одному и тому же ресурсу
- Есть список глаголов трёх видов
  - Безопасные: GET, HEAD, OPTIONS - не меняют состояние ресурсов
  - Идемпотентные: PUT, DELETE - меняют состояние ресурсов, но одинаковым образом, поэтому могут быть вызваны множество раз
  - Небезопасные: POST - не даёт никаких гарантий
