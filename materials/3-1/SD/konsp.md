...
### Dependency Inversion Library framework
...
# 25.02.12 - Лекция
## Продолжаем про метапрограммирование
*Пытаемся придумать, как генерировать код или что-то типа того...*

Beans-нотация для Java-классов несколько упрощает серилазцию и десериализацию
...
```
- `ElementType`:
  - `TYPE` - класс или интерфейс
  - `METHOD`
  - `FIELD`
  - `PARAMETER` - аргумент метода
  - `TYPE_PARAMETER` - generic-тип
...
*Базовый пример использования CGLIB со стр. 27* - основная проблема - необходимость некоторых дополнительных действий для "активации" логики аннотаций

Куда более интересным и ... *Дальше ещё было много чего про разные библиотеки для кодогенерации в Java*

# 25.02.26
## Аспектно-ориентированное программирование
В ООП класс является и единицей модульного разделения, и сущностью из предметной области, а также отвечает за создание модулей - это пересечение определений может привести к переусложнению кода и нарушению многих принципов проектирования

В АОП делается следующее разделение:
- **Аспект** - базовый модуль системы и некая абстракция поведения, которая может относиться к множеству сущностей из предметной области
- **Класс** представляет собой композицию аспектов (уже не минимальная единица модульности) и отвечает за создание объектов

# 25.03.12
## Spring Framework
Фрэймворк, в отличие от библиотеки, содержит не только определённые функции, но и сразу предполагает сценарии их использования (*пример из другой области - каркасное строительство зданий*)

*""Ява"... Первый на моей памяти, кто правильно произносит название жавы, но зачем?..*

*Конфигурация bean-классов через XML... Чего-то более отвратительного я давно не видал*
