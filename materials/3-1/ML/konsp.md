- [Инфо](#инфо)
- [Заметки с консультации](#заметки-с-консультации)
- [Список экзаменационных вопросов (*неточный*)](#список-экзаменационных-вопросов-неточный)
- [Лекция 1](#лекция-1)
  - [Классификация](#классификация)
    - [Метод прецедентов](#метод-прецедентов)
      - [Метод `k`-соседей](#метод-k-соседей)
      - [Парзеновское окно](#парзеновское-окно)
      - [Ядровое сглаживание (kernel-based)](#ядровое-сглаживание-kernel-based)
      - [Заключение](#заключение)
- [Лекция 2](#лекция-2)
  - [(Квази-)линейные методы классификации](#квази-линейные-методы-классификации)
    - [Линейный/квадратичный дискриминант](#линейныйквадратичный-дискриминант)
      - [Случай N переменных](#случай-n-переменных)
      - [Примерные параметры](#примерные-параметры)
    - [Дискриминант Фишера](#дискриминант-фишера)
...
- [Лекция 3](#лекция-3)
  - [Метод опорных векторов](#метод-опорных-векторов)
    - [В случае линейно разделимой выборки](#в-случае-линейно-разделимой-выборки)
    - [Если выборка линейно неразделима](#если-выборка-линейно-неразделима)
      - [Kernel trick](#kernel-trick)
      - [Выводы](#выводы)
- [Лекция 4](#лекция-4)
  - [Логические методы классификации](#логические-методы-классификации)
    - [Решающие списки](#решающие-списки)
    - [Решающие деревья](#решающие-деревья)
      - [Критерии (в т.ч. Gini)](#критерии-в-тч-gini)
- [Лекция 5](#лекция-5)
  - [Композиция (ансамбли) решающих функций](#композиция-ансамбли-решающих-функций)
    - [Независимые подвыборки (random forest)](#независимые-подвыборки-random-forest)
    - [Бустинг](#бустинг)
      - [AdaBoost](#adaboost)
  - [Ансамбли разных решающих функций](#ансамбли-разных-решающих-функций)
    - [Простое голосование](#простое-голосование)
    - [Стэкинг](#стэкинг)
- [Лекция 6](#лекция-6)
- [Лекция 7](#лекция-7)
  - [Оценивание качества решений](#оценивание-качества-решений)
    - [Контрольная выборка](#контрольная-выборка)
      - [Точечная оценка](#точечная-оценка)
      - [Доверительный интервал](#доверительный-интервал)
      - [Байесовский подход и уравнивание](#байесовский-подход-и-уравнивание)
      - [Выводы](#выводы-1)
    - [Эмпирический риск](#эмпирический-риск)
      - [Оценки Вапника-Червоненкинса](#оценки-вапника-червоненкинса)
      - [Выводы](#выводы-2)
    - [Скользящий экзамен (leave-one-out, hold-out, кросс-валидация и т.п.)](#скользящий-экзамен-leave-one-out-hold-out-кросс-валидация-и-тп)
      - [Несмещённость кросс-валидации](#несмещённость-кросс-валидации)
      - [Ещё некоторые методы скользящего экзамена](#ещё-некоторые-методы-скользящего-экзамена)
      - [Свойства](#свойства)
    - [Выводы](#выводы-3)
- [Лекция 8](#лекция-8)
  - [Критерии качества](#критерии-качества)
    - [Критерии точности классификации](#критерии-точности-классификации)
      - [ROC-кривая](#roc-кривая)
    - [Для непрерывной функции](#для-непрерывной-функции)
  - [Разложение ошибки](#разложение-ошибки)
    - [Bias-variance decomposition (разложение bias)](#bias-variance-decomposition-разложение-bias)
    - [Теория статистической устойчивости](#теория-статистической-устойчивости)
...
1. Разложение bias
   - [Тык](#bias-variance-decomposition-разложение-bias)
2. Precision-recall кривая
   - [Тут есть](#критерии-точности-классификации) ссылка на статью
3. Random forest - два основных принципа
   - [Тык](#независимые-подвыборки-random-forest)
4. ROC-кривая
   - [Тык](#roc-кривая)
5. Теорема о несмещаемости кросс-валидации (формула в первую очередь)
   - [Тык](#несмещённость-кросс-валидации)
6. Что такое бустинг? Как эта идея реализована в AdaBoost?
   - [Тык](#бустинг)
7. Формула разделяющих кривых при нормальном распределении 2 классов и разных ковариационных матрицах
   - [Тык](#линейныйквадратичный-дискриминант) (конкретно вот [этот раздел](#случай-n-переменных))
8. Формула логистической регрессии (сигмоид)
   - [Тык](#логистическая-регрессия)
9. В каком случае эмпирический риск (частота ошибок на обучающей выборке) является состоятельной оценкой риска (вероятности ошибки)? Запишите в мат. нотации
   - Смотри выводы к [этому разделу](#эмпирический-риск)
10. Основная идея SVM
    - [Тык](#метод-опорных-векторов)
11. Напишите формулу, подтверждающую гипотезу Наивного Байесовского классификатора
    - [Тык](#наивный-байесовский-классификатор)
12. Примеры выбросов, к которым SVM устойчив, а линейный дискриминант - нет
    ...
    - ...
13. Напишите формулу критерия Gini (критерий ветвления деревьев)
    - [Тык](#критерии-в-тч-gini)
    - Квадрат выводится из формулы среднего для вероятности ошибочного результата
14. Что такое переобучение и недообучение?
    - ...
15. Основные параметры бустинга? Нарисовать зависимость точности модели от этих параметров
    - [В целом о бустинге](#бустинг)
    ...
    - ...
16. Почему бустинг лучше нейронных сетей на табличных данных?
...
17. Как связаны бустинг и регрессия?
    - *Ответ с консультации:* Бустинг может быть выражен через логистическую функцию, если мы настроим деревья так, что их результаты будут достаточно независимы
18. Kernel trick
    - [Тык](#kernel-trick)
...
$$
R(c, \lambda) = \int_D L(y, \lambda(x)) P_c(dx, dy)
$$
...
*Опуская душный матан, можно сказать, что это та область, в которой могут находиться резлуьтаты модели (откровенно говоря, этот ответ не говорит вообще ничего, так что лучше ещё раз о нём перечитать)*
...
*Дальше шла ещё какая-то муть про случаи, когда функция потерь - не индикаторная функция, вывели пару "полезных" формул и устновили, что делить контрольную выборку на фолды бессмысленно*
...
Если будем брать частоту ошибок $\nu = \frac{N_e}{N}$, то вероятность того, что $\nu$ отличается от вероятности ошибки $p$ на $\epsilon$ будет $< e^{-2\epsilon^2N}$

Если у нас будет $L$ решающих функций, то получим $\epsilon \approx \sqrt{\frac{\ln L}{2 N}}$

Ввиду бесконечного количества решающих функций за $L$ обычно берут количество классов эквивалентности решающих функций

*Вся эта херабора вроде как доказывает **состоятельность** эмпирического риска (а далее нам говорят, что практической бользы от этого мало... Ну заебись...)*

*Затем идёт подраздел, который даже Неделько советует скипать (я проигнорирую его уточнеие, что "при первом прочтении"). Вывод из этого раздела - оценка такая будет неулучшаема, пока у нас нет новой информации*
...
Говоря человечески, матожидание риска k-fold кросс-валдиации будет равно матожиданию риска для выборки, из которой мы исключили один число элементов, содержащееся в одном фолде
...
- Матожидание суммы - это суммма матожиданий
...
[Подробнее тут (в т.ч. про precision-recall-кривую)](https://education.yandex.ru/handbook/ml/article/metriki-klassifikacii-i-regressii)

#### ROC-кривая
ROC-кривая - кривая ошибок. Отражает зависиомость верно- и ложноположительных ответов.
...
[Подробнее](https://habr.com/ru/companies/otus/articles/809147/)
...
Ответ от GPT:
Получаем «шум», смещение и разброс (в точке x).
1. Du — шум:
...
3. Dv — разброс (varience):
   ...
   - Это отражает чувствительность модели к обучающим данным. Если модель слишком сложная (например, переобучилась), то её предсказания сильно зависят от конкретной выборки.
