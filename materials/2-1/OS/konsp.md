...
## Вахалия - "Unix изнутри", глава 4 - Сигналы и управление сеансами
...
## Job control для shell'а
[Источник](https://www.gnu.org/software/libc/manual/html_node/Job-Control.html)

Job - группа процессов (причём зачастую совпадает с группой), которая является единицей управления терминальной сессии

Необходимо контролировать доступ джобсов к управляющему терминалу
...
`open(char* pathname, int flags, [mode_t mode])` - Открывает файл по указанному пути и с указанными флагами, в случае успеха возвращает индекс, указывающий на элемент массива в user area - указатель на дескриптор файла.
...
Отображает участок файла в виртуальное адресное пространство процесса, причём при исполнении в ядре будет создана особая структура (*на лекции назвал её **сегментом**, но я бы так не говорил, чтобы не путаться с сегментами при виртуальной адресации*), хранящая адрес в виртуальной памяти и связанный с ним источник подкачки. При попытке прочитать файл, ядро подкачает нужный файл методом, похожим на страничный обмен.

На самом деле все участки в памяти процесса выделяются `mmap`. Посмотреть их можно командой `pmap <pid>`
...
```c
while(flag) {} // Ожидаем, пока флаг не станет ложью
flag = 1;
// Критическая секция
flag = 0;
```
...
Контекст именно процесса должен также включать состояние диспетчера памяти, нити в рамках одного процесса могут не сохранять состояние менеджера
...
Переход между состояниями осуществляет программный модуль **планировщик**, при этом сам планировщик не является нитью и должен использовать более простые примитивы синхронизации. Соответственно, планировщик также отвечает за сохранение и восстановление контекста.
...
Главным преимуществом будет детерминизм и то, что race condition будет ловиться тестированием.

#### Недостатки кооперативной многозадачности
- При ошибках программирования или в некоторых других случаях нить может не отдавать управление никогда
...
**Разделённое время** - оптимизированное среднее время реакции на внешнее событие

### Реального времени
- Обычно используют фиксированные приоритеты
- Расписываются требования к времени реакции для всех задач
- Задачам с меньшим временем даём больший приоритет
...
- *Остальные вызовы смотри в [конспекте Вахалии](#svr4)*
...
Существуют 2 типа каналов: именованные и неименованные.

## `pipe(2)`
`int pipe(int fildes[2])` - создаёт неименованный канал и записывает его дескрипторы в переданный массив. В стандарте UNIX первый дескриптор на чтение, а второй - на запись. В Solaris оба дескриптора двунаправленные и позволяют читать и писать перекрёстно

### Взаимодействие с программынми каналами через разные системные вызовы
- `open` - не нужен
- `close` - используется, когда работа с трубой закончена. Если мы закрыли дескриптор на запись, другой конец при чтении получит `EOF`, при попытке чтения. **ЗАКРЫТЬ НЕОБХОДИМО ВСЕ КОНЦЫ ТРУБЫ**, причём самым первым делом надо закрыть концы, которые не будут использовать в этом подпроцессе
...
## `popen(3)`
Функции СИшной библиотеки позволяют использовать обёртки в таком обрезанном, но более удобном виде: не нужно вручную форкаться и следить за всеми дескрипторами.

- `FILE *popen(const char* command, const char* type)` - открывает трубу на режим `type` и запускает процесс-потомок командной `command`, используя `/bin/sh`, в котором другой конец трубы будет подсоединён к `stdin` либо `stdout`
- `int pclose(FILE *pipeStream)` - закрывает трубу и вызывает `wait` для процесса, связанного с другим концом этой трубы
- **ТОЛЬКО НА СОЛЯРИСЕ**:
  - `int p2open(const char* command, FILE* fp[2])` - создание труб для двух направелний с подсоединением к их разным концам подпроцесса `command`
  - `int p2close(FILE* fp[2])` - закрывает трубы и ожидает завершение подпроцесса

## Именованные трубы (FIFO-файлы)
`int mknod(const char *path, mode_t mode, dev_t dev)` - создаёт файл произвольного типа

При этом для не-`root` разрешно лишь создание FIFO-файлов (`mode = S_IFIFO | access_rights` - причём `mode_t` здесь рассматривается полностью, смотри в [`umask`](#umask2))
...
Сокеты - основной интерфейс для сетевого взаимодействия, хотя могут использоваться и процессами на одной машине

**Для работы с сокетами необходимо подключать библиотеку `"socket.h"`, используя при компиляции `-lsocket`**

"Всё - файл" постепенно преобразуется в "Всё - сокет"

**Потоковые сокеты** предназначены для передачи потока байтов 

Для использования сокета необходимо указать его предназначение. Как это делается, будет показано далее

Unix domain socket используется для связи в рамках одной машины.

TCP сокеты могут использоваться как для сетевого, так и для локального взаимодействия

### `socket(3SOCKET)`
`int socket(int domain, int type, int protocol)` - создаёт точку связи и возвращает его дескриптор
- `domain` - используемый тип адресации. Определяет формат конечных точек адресации:
  - `AF_UNIX` / `AF_LOCAL` - Unix domain socket. Права доступа к сокетам будут определяться простыми правами доступа файлов
  - `AF_INET` - IPv4
  - `AF_INET6` - IPv6
- `type`:
  - `SOCK_STREAM` - двунаправленный сокет для передачи потока байтов
  - `SOCK_DGRAM` - сокеты для передачи пакетов (датаграмм)
- `protocol` - чаще всего определяется сочетание домена и типа, поэтому зачастую в качестве этого аргумента передают `0`, предоставляя системе возможность выбрать необходимый протокол самостоятельно

*Остальные домены и сокеты смотри в мане*

Полученный таким образом дескриптор уже будет отсылаться на сокет, но пока что мы его можем разве что закрыть

## `bind(3SOCKET)`
`int bind(int sockfd, const struct sockaddr *addr, socklen_t addrlen)` - привязывает сокет к имени, определённому структурой `struct sockaddr`. `addrlen` указывает длину привязываемого адреса в байтах
...
// Версия для работы с доменом AF_UNIX (при передаче в bind необходимо провести приведение типа к struct sockaddr)
#include <sys/un.h>
struct sockaddr_un {
    sa_family_t sun_family;               // AF_UNIX
    char        sun_path[UNIX_PATH_MAX];  // pathname
...
struct sockaddr_in { /* AF_INET */
  ...
  in_port_t sin_port;
  struct in_addr sin_addr; // Адрес в бинарном виде
  char sin_zero[8]; 
};

// Для IPv6 используется struct sockaddr_in6
```

Отдельный интересный вопрос: как определить размер передаваемой в `bind` структуры? Современный подход - `sizeof(addr)`

После исполнения `bind` будет создан файл сокета с правами `0777 & umask`

## `listen(3SOCKET)`
**Сервер** - процесс, который держит слушающий сокет

`int listen(int sockfd, int backlog)` - регистрирует сокет по дескриптору sockfd как серверный, то есть сокет, который будет принимать входящие подключения через `accept()`. `backlog` показывает максимальную длину очереди ожидаемых подключений от клиентов

Перед использованием `listen` должны быть вызваны `socket()` для создания сокета и `bind()` для возможности обращаться к сокету извне
...
Если передать указатели вторым и третьим аргументами, они будут заполнены данными созданного сокета-клиента (*из-за того, что клиентский Unix domain socket никуда не биндится, мы не сможем получить его имя*)

Если очередь запросов пуста и сокет не отмечен как неблокирующий, процесс будет заблокирован, пока не появится запрос к сокету. Если сокет неблокирующий, то `accept()` завершится с ошибкой `EAGAIN` или `EWOULDBLOCK`

Вместо ожидания в блоке можно использовать `select(2)` или `poll(2)`, которые сообщат о появлении запроса в очереди. [Смотри тут](#использование-poll2-для-сокетов)

## `connect(3SOCKET)`
`int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen)` - сооединяет дескриптор сокета `sockfd` с адресом, определённым в структуре `addr`. Для клиентского сокета нам необходимо вызвать для него `socket`, а также знать данные структуры забинженного серверного сокета
...
## Общая цепочка работы с сокетами

## UNIX Domain socket - конфигурация
- Тип - только `SOCK_STREAM`
- Право на соединению регулируется правами доступа к файлу
- После завершения работы сервера, файл сокета останется в системе и его нельзя будет забиндить повторно `=>` надо вызывать для него `unlink`, причём необходимо, чтобы лишь сам сервер при своём завершении делал это

## Сокеты TCP
- Домен - `AF_INET` или `AF_INET6`
- Тип - `SOCK_STREAM`
- Сервер будет идентифицироваться адресо IPv4 либо IPv6 и портом
- Номер порта - 16-битное число
- В Unix слушать на портах `< 1024` может только привилегированный пользователь

# Некоторая теория по сетевому взаимодействию
На самом деле, вместо классической модели OSI сейчас чаще используется DoD:
- *что-то про эту систему надо написать, но пока не нашёл презентацию*

Данные по сети передаются пакетами. Самый распространённый размер пакета 1500 байт.

MAC-адреса - индивидуальное имя устройства. Вообще, должно быть уникальным, что обеспечивается очень специфическим образом.

Для построения современного интернета нужна иерархическая адресация из хоста, сетевого агрегатора и сетевой системы

Пакеты передаются через все уровни и при этом могут потеряться, что порой бывает критично. 

TCP: пакеты, содержащие кусочки одного сообщения, нумеруются. Получатель, получив и собрав все пакеты, отправит отправителю сигнал подтверждения. Если отправитель не получит сигнал подтверждения, то отправит пакеты повторно. После троекратного неподтверждения, соединение будет оборвано

TCP надёжен, но из-за дополнительных проверок работает медленнее. UDP, в свою очередь, допускает потерю пакетов, но передаёт данные куда быстрее (для видео/аудио звонков используются в основном настройки именно над UDP)

## Возвращаемся к TCP сокетам
Маска подсети побозначается как `IP-адрес/количество_бит_в_маске`
...
```c
#include <netdb.h>
struct hostent *gethostbyname(const char *name); // Возвращает указатель на структуру, в которой будут IP-адреса, связанные с именем хоста
// Обращается к DNS-серверу либо /etc/hosts
```
...
// Работаю похожим методом на fcntl, ioctl, а ещё больше похоже, пожалуй, на tcgetattr/tcsetattr
```
Некоторые опции:
- `SO_REUSEADDR` - использовать адрес сокета повторно
- `SO_KEEPALIVE` - хранит соединение живым - раз в какое-то время будет посылаться пакет, не содержащий полезной информации
- `SO_LINGER` - при закрытии сокета будет пытаться дослать ещё неотправленное
- `SO_BROADCAST` - разрешить пересылку широковещательных сообщений
- `SO_SNDBUF` - размер буфера для записи
- `SO_RCVBUF` - размер буфера для чтения
- `SO_NOSIGPIPE` - при попытке записи в закрытй с дрйго стороны сокет не будет присылаться `SIGPIPE`

Если после нескольких обращений, не было получено ответа, сокет будет автоматически закрыт

## Датаграммные сокеты
Такие сокеты чаще всего работают по протоколу `UDP`. Датаграмма может иметь размер чуть меньше 64 кБ. При этом передача всё равно проходит кусочками. При этом для скорости передачи нет надёжного механизма сохранения целостности. Считается желательным посылать в ответ на полученную датаграмму ответ, благодаря чему мы можем решить, надо ли нам отправлять датаграмму повторно или нет.

С использованием UDP мы можем рассылать датаграмму широковещательно или нескольким адресатам.

Протокол UDP позволяет отправлять датаграммы без установления соединения.

Чтение из UDP-сокета читает пакет целиком. Если он не поместится в буфер, то будет обрезан (но пакет будет потерян целиком)

В связи с выской вероятности ошибок при передаче используются следующие методы:
- Отправка пакетов сильно меньше 64 кБ
- Избыточность каждого пакета, которая позволила бы восстановить часть потеряных пакетов

По использованию UDP сокеты не сильно отличаются от TCP, за исключением следующих особенностей:
- `sendto` и `recvfrom` позволяют отправлять/получать данные без `connect`
- `connect` реально не создаст подключение, а просто запомнит адрес. После него можно использовать по классике `read`/`write`, `recv`/`send`
- `bind` используется для установления порта

## Использование `select` и `poll`
*Общее описание уже неплохо сделал [тут](#системный-вызов-poll2). Здесь будут дополнительные примечания*

По стандарту `select` может записывать по указателю `timeval` недоспанное время, хотя в солярисе это не работает

Один из недостактво `select` - это битовые маски, из-за которых мы ограничены по количеству дескрипторов и их максимальному номеру

*В винде тоже есть `select`, но работает несколько иначе + работает только с дескрипторами сокетов*

**Очень важно не добавлять в `select` и `poll` лишние дескрипторы, иначе эти вызовы могут завершаться, не принося никакой полезной информации

`poll` уже достаточно хорош, но его главная проблема - мы можем проверять события только по дескрипторам, но будем всё также блокироваться на `wait` либо ожидании иных примитивов синхронизации

`epoll` - модфикация `poll` в Linux

**Чем отличается фреймворк от библиотеки?** Бибилотека - набор используемых нами функций в своём окружении. Фреймворк в свою очередь настраивает окружение почти полностью, запрашивая от программиста определение некоторой дополнительной логики

Очень рекомендует встроенный в C++ фреймворк OSI (хотя насчёт названия не уверен)

Говорит, что освоить концепцию селектов - **ПРОСТО ИМБА** (*и я даже в это верю, надо будет порешать лабы для множественного соединения*)

# 23.12.11
## Подсистема ввода-вывода
**Бесперебойник** и **батарейка** - одно из ключевых устройств. Считается устройством, потому что она может отвечать, сколько у неё осталось заряда

Многие операционные системы (и в частности, UNIX), запрещают прямой доступ к периферийным устройствам. Прямая работа с ними доступна только в системном режиме

Лозунг "всё - файл" на самом деле имеет смысл заменить на "всё - устройство"

# 23.12.18
## Ещё немного про ввод-вывод
**Блочные устройства** - это в первую очередь диски.

**Синхронный** ввод-вывод - делаем операцию и ожидаем её завершения. Соответственно, при **асинъронном** вводе-выводе мы не ожидаем завершения операции.

Преимущества асинхронного ввода-вывода:
1. Нет необходимости гарантировать доступность буфера чтения-записи, которая реализуется копированием данных в буфер и из него. В асинхронном можно иногда обойтись без копирования
2. Полезен для получения дополнительной информации в системах реального времени

Классический вариант асинхронного ввода-вывода в Unix - использование очереди системных вызовов, которые по очереди поступают в драйвер
...
Один из вариантов организации драйвера ФС - запуск драйвера как пользовательского процесса с привязкой его к ядру через *шлюз* (в лекции он назвал этой просто *штукой*, так что моё слово считаю не хуже)

Самая простая по структуре ФС - архивы **tar** (причём он есть в стандартах POSIX). В файловой системе tar нет каталогов. Каждый файл содержит заголовок с полным путём до него от корня архива, длиной файла и другими его атрибутами. Все поля в заголовке текстовые. После заголовка располагается содержимое самого файла. Далее идёт следующий файл с заголовком. На самом деле, в архиве могут быть каталоги, но они представлены лишь заголовками (то есть путём до каталога), принадлежность файлов к каталогу будет определяться уже путями в заголовках этих файлов.

**tar.gz** - упакованных архив tar (сам по себе tar не упаковывает данные). Причём gzip упаковывает данные как непрерывный поток битов, что с одной стороны делает архив достаточно компактным, но не позволяет удобно и быстро достать из архива отдельный файл.

**zip** - в нём уже есть полноценный центральный каталог. Метаданные хранятся в бинарном виде.

**RT11** - файловая система, использовавшаяся в 16-битных мини-кмопьютерах (*с ОЗУ 64 КБ!!!*). ФС продолжает развивать идеи **zip**:
- Существуют **корневой** каталог, благодаря этому файлы располагаются в памяти диска в том же порядке, в каком лежат в каталоге
- Файлы занимают непрерывное место на диске
- Появилось понятие свободного места (оно обозначалось особым недопустимым символом кадировки radix)
- При создании файла необходимо было указывать длину (топорный, но рабочий способ частичной борьбы с фрагментацией)
- Главное достоинство - простота (*всё ещё*) и возможность влезать (*почти полностью*) в малую память

**ISO 9660**:
- Файлы также располагаются непрерывно (то есть также фиксированной длины)
- Появляются вложенные каталоги, поэтому в заголовках необходимо указывать адрес на начало (*чего? каталога этого файла, корня ФС или...*)

Одна из очень больших проблем описанных выше ФС - непрерывное расположение файлов на диске, из-за чего те очень сложно (либо и вовсе невозможно) расширять. Одно из решений предоставила ФС **FAT**:
- Файл представлен в виде связаного списка, но указатели списка лежат не в блоках файла, а в таблице FAT (file allocation table), в которой есть по одной записи на каждый блок диска (512 байт). Если блок принадлежит файлу, то запись в таблице будет содержать адрес следующего блока того же файла, в ином случае блок может быть помечен как свободный (`0`) или плохой. Последний блок файла помечается записью `-1`
- Самый старый вариант - FAT12 - 12 бит => 4096 блоков => 2 МБ (дискеты)
...
Для уменьшения внутренней фрагментации было предложено решение укладывать концы файлов в один кластер (но идея эта большой популярности не снискала)

# 23.12.25
### FAT
- Все метаданные файлов хранятся в каталоге
- При работе с одним файлом из каталога мы должны прочитатть данные и для других файлов в каталоге
- По этим причинам увеличение объёма метаданных файлов очень нежелательно

Решением стал перенос атрибутов файлов в особые структуры (например, inode в UFS), в метаданных теперь стало возможно хранить только ссылку на эту особую структуру и имя файла

Современные файловые системы:
- **FAT12/16/32**
  - Поддерживается большинством ОС, в том числе и не компьютерами (видокамеры и т.п.), поэтому часто используется для флэш-накопителей
  - **ExFAT** - оптимизированная для флэш ФС
- **NTFS** - основная ФС Windows
- **UFS** - основная ФС BSD Unix, поддерживается в Solaris
- **Ext2**/**Ext3** - основные ФС Unix  девяностых и нулевых
- **Ext4** - основная ФС в современных Unix-системах
- **ReFS** - новая ФС от Майкрософт, инфы очень мало
- ФС с копированием при записи
  - **NetAPP WAFL**
  - **Sun/Oracle**
  - *чего-то тут ещё надо добавить*

В современных ФС каталог представляет собой список из имён файлов и номеров их инод. А вот уже в иноде в свою очередь будут храниться все метаданные файла и куча всех других операций. Такой подход позволяет нам работать с конкретными файлами из каталогов и не читать инфы по ненужным нам файлам. Вдобавок, у нас нет необходимости держать в памяти всю таблицу FAT.

В некоторых системах иноды не хранятся в таблице, а пишутся в произвольные места на диске, а номер иноды указывает на блок на диске. Такой подход позволяет не беспокоиться о заполнении или чрезмерном разрастании таблицы инодов до того, как место на диске на самом деле закончилось (теоретически, этого можно достигнуть, создавая много маленьких файлов)

Рассказывает про правило 13 (10, 14, 15 или ещё сколки-то) указателей в **Ext3**:
- Начальные указатели хранят информацию о файле
- 10 прямых указателей на блоки файла
- Если файл не влезает в 10 блоков, то 11-м идёт косвенный блок - указатель на блок данных, целиком состоящий из указателей, при недостатке и этого количества указателей на блоки диска могут быть блоки двойной или тройной косвенности

Про блочные деревья и экстенты (*это надо ещё раз переслушать*)

Основные проблемы современных ФС:
- Список свободных блоков и списки блоков файлов - это разные структуры.
  - Нельзя обновить оба списка сразу атомарно
  - Может возникать проблема с потерянными или дважды учтёнными блоками
- При сбоях питаниях мы никак не сможем защититься

Решения проблемы консистентности:
- Старое решение: dirty flag и проверка файловой системы
  - При записи в ФС взводится dirty flag. Если при последующих запусках будет обнаружен dirty flag, мы заключаем, был ли сбой. Далее проводится восстановление ФС методом, похожим на mark&sweap. Все недоступные блоки будут далее считаться свободными
- Новое решение: журналирование
- Cpy-on-write (log structured) ФС

### Журнальные ФС
**Транзакция** - операция или группа операций, которые либо происходят полностью, либо не происходят вовсе

<hr>

Для журналируемых ФС определяются критерии целостности только для метаданных. Таким образом, мы можем определеить критерии для проведения транзакций, но остаётся вопрос: что делать при выключении электричества.

**Журнал** - место на диске (не обязательно на том же, находится ФС). Линейное хранилище.

В ходе транзакции мы:
- Записываем информацию о транзакции в журнал
- Помечаем конец транзакции меткой COMMIT
- Дожидаемся, пока описание транзакции физически ляжет на диск
- Начинаем накатывать изменение на саму ФС
- Вносим в журнал запись SYNC

В итоге при сбое проводится проверка журнала. При этом есть 3 случая:
- SYNC - всё записано, всё хорошо
- нет ни SYNC, ни COMMIT - ничего не успело произойти, всё хорошо
- есть COMMIT, но нет SYNC - надо откатить (un-do) либо завершить транзакцию (re-do)
  - un-do - надо хранить в журнале изначальное состояние файлов
  - re-do - операции в транзакции должны быть идемпотентными, то есть при многократном совершении операции результат будет тот же, что и при однократном

**Плюсы** журналов:
- Фиксированное и обычно небольшое время восстановления ФС после сбоя
- Можно более агрессивно кэшировать данные (сказать пользователю, что мы всё записали, хотя на самом деле этого не дслелали, а сделаем чуть позже в фоне)

**Минусы**:
- Метаданные пишутся на диск дважды (в журнал и в реальное место)
- Код ФС усложняется
- Появляются сценарии повреждения ФС, которых нет в традиционных версиях. Например, запись корректных данных на диск и некорректных в журнале (баг драйвера)