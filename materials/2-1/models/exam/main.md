...
**Модуль сходимости** $m(x)$ - функция, возвращающая нам для каждого икса одно из $s$, для которого наблюдается равенство $f_s(x) = f(x)$
...
- $N_{\alpha \rarr \beta} = N_{\alpha} \rarr N_{\beta}$ (вот теперь стрелочка для множеств начинает выглядеть логично)
...
- Для частичных функций может работать некорректно, например, для ЧВФ $f(x)$ функция $0(f(x))$ в то время как в лямбдах это будет всюду определённая константа. Решается проблема определением функции как $f_0(\vec{x}) = f(\vec{x}) * sg(s(g_1(\vec{x}) + ... + g_n(\vec{x})))$ - теперь функция $f_0$ будет корректно сохранять частичность при записи через термы
...
- $Q_1 = y^{\sigma \rarr \tau}$
- $Q_2 = \lambda x^{\sigma}. (y^{\sigma \rarr \tau} x^{\sigma})$
...
- $M \equiv (N^{\sigma \rarr \tau} P^{\tau})$: предположим, что во внутренних термов содержится свободная переменная такая, что $\delta(x^{\rho}) \in R_{\rho}$, тогда по предположению индукции $[[N]]_{\delta} \in R_{\sigma \rarr \tau}, [[P]]_{\delta} \in R_{\sigma}$, а значит из определения $R_{\sigma \rarr \tau}$ заключаем, что $[[M]] \in R_{\tau}$
- $M \equiv \lambda x^{\sigma}. N^{\tau} : \sigma \rarr \tau$. Логично, что $[[M]]_{\delta} = app^{-1}(a \rarr [[N]]_{\delta[x^{\sigma} \rarr a]}) : D_{\sigma} \rarr D_{\tau}$, то есть чтобы доказать $[[M]]_{\delta} \in R_{\sigma \rarr \tau}$ надо доказать, что $[[M]]_{\delta}(a) = [[N]]_{\delta[x^{\sigma} \rarr a]} \in R_{\tau}$ при любых $a \in R_{\sigma}$. Первое верно по индукционному предположению, а второе будет верно в силу обозначения $\delta[x^{\sigma} \rarr a](x^{\sigma}) = a \in R_{\sigma}$ (учитывая, что $x^{\sigma}$ свободно для $N$), отсюда и заключаем
...
Элементы $f \in D_{\sigma}$ для модели Хенкина будет **$\lambda$-определиммым**, если существует замкнутый терм $M$: $[[M]] = f$

**Логическое отношение** $\R = \{ R_{\sigma} \}$ - это ЛП намножестве моделей Хенкина, где $R_{\sigma} \subseteq D_{\sigma}^{(1)} \times ... \times D_{\sigma}^{(n)}$. *По факту, можно воспринимать ЛО как ЛП для модели Хенкина, у которой $D_{\sigma} = D_{\sigma}^{(1)} \times ... \times D_{\sigma}^{(n)}$ $\rArr$ к ЛО также применима основная теорема об ЛП

**Порядоковое число $o(\alpha)$**:
- $o(\alpha) = 0$, если $\alpha$ - атомарный тип
- $o(\alpha \rarr \beta) = \max(o(\alpha) + 1, o(\beta))$

# Синтаксис PCF
*Выглядит, как бесполезная херня, но при этом вроде не очень сложная*

PCF - простой язык типов с двумя атомарными типами: $\omega$ для наутральных чисел и $\beta$ для буелвы значений. Сложны типы определяются индуктивно также, как и лямбда-типы

Теперь можем здать функции `ISNULL`, `SUCC`, `PRED`, `COND`, `Y`

PCF-термы определяются индуктивно также, как и лямбда-термы

`eval(M)` применяет все возможные правила и редукции к `M`, приводя его к `N`, если терм ненормализуем, значение функции не определено
...
**Плоский домен:** $(M_{\perp}, \sqsubseteq)$, где $x \sqsubseteq y \hArr [(x = y) \vee (x = \perp)]$
...
**$\omega$-цепь** ЛУМ из натуарльных чисел
...
- **Минимальным элементом**, если $\forall x \in A : x \sqsubseteq a \rArr x = a$

*Аналогично для наибольшего и максимального*

- Для ЛУМ эти понятия совпадают
- Любой наименьший элемень будет единственным минимальным, обратное не верно
...
**Точная верхняя грань (супремум)** - наименьшая верхняя грань. Обозначается $\sup(A) = \sqcup A$. Точная нижняя грань (инфенум) определяется аналогично (и обозначается $\inf(A) = \sqcap A$). **Точных граней может и не быть**
...
## Теорема о неподвижной точке
**Т.** Пусть $D$ - омега-домен, а $f$ - омега-непрерывная функция, тогда $f$ имеет нименьшую неподвижную точку $Yf = \sqcup_{n \in \N} f^n(\perp)$

*Доказательство:*
- Из монотонности вытекает, что $\perp \le f(\perp) \le f(f(\perp)) = f^2(\perp)$, теперь надо доказать, что $f^n(\perp) \le f^{n+1}(\perp)$
- У нас получается цепь $\perp \le f(\perp) \le f^2(\perp) \le ... \le f^n(\perp) \le ...$, имеющая точную верхнюю грань в силу доменности.
- Положим, что $\sqcup_{n \in \N} f^n(\perp) = x_0$, тогда в силу омега-непрерывности: $f(x_0) = f(\sqcup_{n \in \N} f^n(\perp)) = \sqcup_{n \in \N} f^{n+1}(\perp) = x_0$, то есть $x_0$ - неподвижная точка $f$. 
- Пожкаем теперь, что это наименьшая неподвижная точка, взяв другую неподвижную точку $x_1$, причём $\perp \le x_1$, а значит в силу монотонности $f$ получаем, что $\sqcup_{n \in \N} f^n(\perp) \le \sqcup_{n \in \N} f^n(x_1) = x_1 \rArr x_0 \le x_1$, а значит $x_0$ - наименьшая неподвижная точка

## Системы функций и домены
**Прямое произведение** доменов $(D_j, \le_j)_{j\in J}$:
$$
\prod_{j \in J} D_j = \{ f : J \rarr \cup_{j \in J} D_j | \forall j \in J : f(j) \in D_j \}
$$
*Говоря простыми словами, это множество всех таких функций, которые по индексу домена возвращают элмент из него*

Если $\forall j \in J : D_j = D$, то $\prod D_j$ обозначается $D^J$ и называется **прямой степенью**

Порядок для функций из прямого произведения определяется достаточно логично: $f_1 \le f_2 \hArr f_1(j) \le f_2(j)$ для всех $j \in J$

*Далее для простоты буду писать просто $\prod$ вместо $\prod_{j \in J} D_j$*

$(\prod, \le)$ - домен с наименьшим элементом $\perp = (\perp_j)_{j \in J}$, причём для любой цепи $A \subseteq \prod$ имеем $\sqcup A(j) = \sqcup_{f \in A} f(j)$ для всех $j \in J$ (*сначала думал, что запись какая-то странная, а потом понял, что она ведь следует из определения*)

**Каноническая проекция** $pr_{j_0} : \prod \rarr D_{j_0}$ будет отображать функции из прямого произвдения доменов в функции одного конкретного домена. Будут непрерывны в силу свойств домена

### Домен функций (Если D, E - домены, то их непрерывное отображение - тоже домен)
Для произвольного множества $D$ и домена $(E, \le_1)$ можно определить отношение порядка для множества их отображений $f : D \rarr E$: $f \le_0 g \hArr \forall x \in D : f(x) \le_1 g(x)$. $\le_0$ будет называтья **поточечным порядком** (*будем обозначать $F = (D \rarr E)$*)

$(F, \le_0)$ - домен с:
- Наименьшим элементом $const_{\perp}$
- Если $\{f_i\} \subseteq F$ - цепь, то $\forall x \in D : \{ f_i(x) \} \subseteq E$, причём $\{ f_i(x) \}$ - тоже цепь, при этом супремум цепей при любых иксах будет совпадать

Если $(D, \le_2)$ - также домен, то супремум цепи непрерывных функций из $F$ также будет непрерывной функцией

*Доказательство:*
- **Непрерывность** - из непрерывности функций из $F$ и доменности $D$ следует, что $\forall x, y \in D : x \le_2 y \rArr f(x) = \cup f_i(x) \le_1 f_i(y) = f(y)$
- **Направленность** - возьмём цепь $\{x_i\} \subseteq D$, надо доказать, что $f(\cup x_i) = \cup f(x_i)$, что следует из природы доменов $D$ и $E$ (*ну правда! Не вижу смысла выписывать равенство, доказывающее это, но если вдруг что, то мы там исходим также из того, что $f(x) = \cup f_i(x)$*)

**С.** Отсюда заключаем, что $([D \rarr E], \le_0)$ - **домен относительно поточечного порядка**

### Непрерывность функций
*Приведу без доказательства эту лемму:* для доменов $D_1, D_2, E$ функция $f_1^D \times D_2 \rarr E$ непрерывна $\hArr$ она покомпонентно непрерывна, то есть если непрерывны функции:
- $\forall y_0 \in D_2 : x \rarr f(x, y_0) : D_1 \rarr E$
- $\forall x_0 \in D_1 : y \rarr f(x_0, y) : D_2 \rarr E$

#### app
Для доменов $D, E$ функция $app : ([D \rarr E] \times D) \rarr E$, то есть $app(f, x) = f(x)$ будет непрерывна

*Доказательство:* (опираясь на лемму выше)
- $\forall f \in [D \rarr E] : x \rarr f(x)$ непрерывна (*неиронично очевидно*)
- $\forall x \in D : f \rarr f(x)$ непрерывна в силу того, что для любой цепи из $[D \rarr E]$ поточечная верхняя грань определяется поточечно ($(\cup f_i)(x) = \cup f_i(x)$) (*ну короче, тоже в силу определения домена $[D \rarr E]$*)

#### comp
Для доменов $D, E, F$ (**не путать с введённым мной ранее доменом $F$**). $id_D : D \rarr D$ непрерывна и если непрерывны $f : D \rarr E, g : E \rarr F$, то композиция $fg : D \rarr F$ также будет непрерывна (*легко доказывается возможностью протащить супремум от иксиов наружу*)

$comp : ([D \rarr E] \times [E \rarr F]) \rarr [D \rarr F]$ - непрерывный функционал (*от каждого компонента композиции протаскиваем супремум наверх, а далее применяем лемму о поточечной непрерывности)

#### it
$it^{(2)} : [D \rarr D] \rarr [D \rarr D] : f \rarr ff$ - непрерывный функционал, равно как и $it^{(n)} : f \rarr f^n$

*Пузырь любезно предложил доказать это в качестве упраженяни и, на самом деле, будто бы тут достаточно сказать, что выражается этот функционал целиком через композицию, а значит в силу непрерывности $comp$ он тоже непрерывен*

*Ну и для случая $n > 2$ можно расписать более длинное протаскивание супремума либо сказать, что определяться будет рекурсивно*

#### Y
$Y_D^{(n)} : [D \rarr D] \rarr D : f \rarr f^n(\perp)$ (*то есть возвращает наименьшую неподвижную точку для функции-аргумента*) - непрерывный функционал

*Доказательство:*
- Отмечаем, что $Y^{(n)} = it^{(n)}(\perp)$, то есть представляет собой композицию непрерывных функционалов
- При этом $Y^{(1)} \le Y^{(2)} \le ... \le Y^{(n)} \le ...$ - омега-цепь, а значит $Y = \cup Y^{(i)}$ (*выглядит так, будто это 2 независимых доказательства*)