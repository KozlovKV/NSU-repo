...
- [24.05.21 - семинар](#240521---семинар)
  - [Алгоритм унификации термов (неформальный)](#алгоритм-унификации-термов-неформальный)
  - [Представление функции через лямбда-терм](#представление-функции-через-лямбда-терм)
- [24.05.28 - семинар](#240528---семинар)
...
Пусть есть частичная функция $g(\vec{x}, z)$. В результате применения оператора мимимизации $M$, получаем функцию $f(\vec{x})$ такую, что $f(\vec{x}) = y \hArr \forall i < y$ ($g(\vec{x}, i)$ определена и не равна нулю) и $g(\vec{x}, y) = 0$
...
Функция $f(\vec{x})$ получается из функции $g(\vec{x}, y)$ и $h(\vec{x})$ с помощью ограниченного $\mu$-оператора, если $g(\vec{x}, y) = 0$ определено для всех $\vec{x}$ и значение $y < h(\vec{x})$, тогда $f(\vec{x}) = y$
...
**Теорема о неподвижной точке**. Для каждой $m+1$-местной ЧВФ $h$ найдётся $m$-местная инъективная ВФ $g$ такая, что:
$$
...
- *И тут ещё 2 пункта, которые опишу... Ну, когда-нибудь опишу*

# 24.05.21 - семинар
## Алгоритм унификации термов (неформальный)
Происходит по построению - рассматриваем подтермы и на каждом шаге проверяем, можно ли унифицировать

## Представление функции через лямбда-терм
*Напоминание о [числах Чёрча](#числа-чёрча)*

Частичная функция $f: \omega^k \rarr \omega$ представима с помщью лямбда-терма F, если для любых натуральных чисел выполняется:
- $f(\vec{n}) \downarrow$, то $((((F \underline{n_1}) \underline{n_2})...)\underline{n_k}) \rArr \underline{f(\vec{n})}$

Кое-какие термы:
- $1^{st} = \lambda z.(t \lambda x . \lambda y . y)$
- $2^{nd} = $

# 24.05.28 - семинар
Наибольший и наименьший может быть один либо нисколько и должен быть сравним со всеми элементами множества

Минимальных и максимальных элементов может быть много, но они не могут быть сравнимы друг с другом